...
def get_actor_from_world(ego_vehicle: carla.Vehicle, dis_limitation=100):
    ...
    # 获取仿真器中所有对象(车辆和行人)， filter("vehicle.*") 筛选出以"vehicle."开头的对象，即筛选出所有的车辆对象。
    vehicle_list = carla_world.get_actors().filter("vehicle.*")  # 获取仿真器中所有的车辆对象，并将其存储在列表中
    for vehicle in vehicle_list:
        ...
        if dis < dis_limitation and ego_vehicle.id != vehicle.id:
            v1 = np.array([vehicle.get_location().x - vehicle_loc.x,
                           ...
                           vehicle.get_location().z - vehicle_loc.z])  # 其他车辆相对于自车ego_vehicle的矢量
            ...
            ego_vehicle_theta = ego_vehicle.get_transform().rotation.yaw * (math.pi / 180)  # 角度制转换为弧度制
            n_r = np.array([-math.sin(ego_vehicle_theta), math.cos(ego_vehicle_theta), 0])
            if -5 < np.dot(v1, n_r) < 5:  # v1在n_r方向上的投影
                if np.dot(v1, ego_vehicle_velocity) > -20:  # 在ego车后10m及车前的视为障碍物  -10
...
def motion_planning(conn):
    while 1:
        ...
        local_frenet_path_opt_ = planning_utils.smooth_reference_line(local_frenet_path_)  # type:
...
if __name__ == '__main__':
    ...
    agent.set_target_speed(10.0)  # 30.0

    """路径规划"""
    # 1. 规划路径，输出的每个路径点是一个元组形式[(wp, road_option), ...]第一个元素是carla中的路点，第二个是当前路点规定的一些车辆行为
    ...
    speed = []
    target_speed = []
    max_speed = 30  # 假设正常行驶速度是40km/h，市区允许的最大速度是50km/h, 前方的动态障碍物行驶的速度是30 km/h.
    # 一旦选择超车，自车可以将速度提升到50km/h  50
    # 设定一个观察者视角
    ...
    while True:
        ...
        #                  size=0.05, color=carla.Color(0, 0, 255), life_time=0.1)  # 预测点为蓝色
        # debug.draw_point(carla.Location(Controller.Lat_control.x_pro, Controller.Lat_control.y_pro, 2),
        #                  size=0.05, color=carla.Color(0, 255, 0), life_time=0.1)  # 投影点为绿色

        # 车道上下边界
        debug.draw_point(carla.Location(),size=0.1,color=carla.Color(0,0,255),life_time=0.5)  # 蓝色
        debug.draw_point(carla.Location(), size=0.1, color=carla.Color(0, 255, 0), life_time=0.5)
