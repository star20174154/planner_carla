...
def frenet_2_x_y_theta_kappa(plan_start_s, plan_start_l, enriched_s_list: list, enriched_l_list: list,
                             frenet_path_opt: list, s_map: list):
    """  已验证
    首先进行必要的坐标转换  frenet -> 直角坐标系
    ...
    """
    target_xy = []
    # 确定规划起点在s_map中的索引,并加入目标路径中     计算起始位置的投影点的信息
    proj_x, proj_y, proj_theta, proj_kappa, pre_match_index = cal_proj_point(plan_start_s, 0, frenet_path_opt, s_map)
    nor_v = np.array([-math.sin(proj_theta), math.cos(proj_theta)])  # 法向量  normal vector
...
def cal_proj_point(s, pre_match_index, frenet_path_opt: list, s_map: list):
    """
    确定给定的s在参考线上的投影点的路径信息    计算投影点  projection point
    匹配点  match points
    param:  s: 要计算投影点的位矢s
    ...
    # 这里存在一点问题，如果动态规划采样点过长，会超出s_map的范围(在frenet_2_x_y_theta_kappa中解决)
    while s_map[start_s_match_index + 1] < s:
...
def Quadratic_planning(l_min, l_max, plan_start_l, plan_start_dl, plan_start_ddl, dp_sampling_res=2,
    ...
    f = w_cost_centre * f
    """二次规划"""
    # cvxopt convex optimization 凸优化求解器
...
def enrich_DP_s_l(DP_s_list, DP_l_list, plan_start_s, plan_start_l, plan_start_dl, plan_start_ddl, resolution=1):
    ...
    end_dl = 0
    end_ddl = 0
    coeffi = planning_utils.cal_quintic_coefficient(start_l, start_dl, start_ddl, end_l, end_dl, end_ddl, start_s,
                                                    end_s)
...
def cal_start_cost(obs_s_list, obs_l_list, begin_s, begin_l, begin_dl, begin_ddl,
                   cur_node_row, row, sample_s, sample_l, w_cost_collision,
                   w_cost_smooth, w_cost_ref):
    """  已验证
    计算 起点 的cost
    参数和DP_algorithm基本一致
